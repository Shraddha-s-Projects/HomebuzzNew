...
export class GoogleMapComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public sameLatLongProperties: Property[] = [];
  public hotSameLatLongpropertyCount: number;
  public url: string;
  ...
  bindGoogleMap(Latitude: number, Longitude: number, type: string) {
    // console.log("bind google map called");
    let zoomLevel = 15;
    let sessionData;
    if (this.localStorage.getItem("SearchData")) {
      sessionData = JSON.parse(this.localStorage.getItem("SearchData"));
  ...
  setMarkerOnGoogleMap(map: google.maps.Map, isZoomAction?: boolean, isHide?: boolean) {
    ...
    propArray.forEach(element1 => {
      element1.IsAddedMultiLabel = false;
    });
      ...
      if (element.LatitudeLongitude != "null"
        ...
        if (_this.pageName === "SearchResultPage" || _this.pageName === "PropertyImageGalleryPage") {
          ...
          if (RankingStatus) {
            ...
            if ((this.pageName == "SearchResultPage" && (!element.IsSameLatLong || (element.IsSameLatLong && !element.IsAddedMultiLabel))) || this.pageName == "PropertyImageGalleryPage") {
              HTMLMarkerFunction.prototype.onAdd = function () {
                ...
                if (_this.pageName === "SearchResultPage") {
                  // start logic google map same lat-long duplicate properties
                  if (element.IsSameLatLong) {
            ...
            if (element.IsSameLatLong && !element.IsAddedMultiLabel) {
              let sameProperties = this.properties.filter(t => t.LatitudeLongitude == element.LatitudeLongitude);
              sameProperties.forEach(prop => {
                let index = this.properties.findIndex(t => t.PropertyDetailId == prop.PropertyDetailId);
                if (index > -1) {
                  this.properties[index].IsAddedMultiLabel = true;
                }
              });
            }
          }
  ...
  openInfoWindowOnGoogleMapMarkerClick(
    ...
    _this.sameLatLongProperties = _this.properties.filter(t => t.LatitudeLongitude == element.LatitudeLongitude);
    infoWindowType = _this.sameLatLongProperties && _this.sameLatLongProperties.length > 0 ? "list" : "single";
    if (infoWindowType == "list") {
      _this.isGoogleMapMarkerListClicked = true;
      _this.isGoogleMapMarkerClicked = false;
    } else {
      _this.isGoogleMapMarkerListClicked = false;
      _this.isGoogleMapMarkerClicked = true;
    }

    let classObjList: HTMLCollectionOf<HTMLElement> = _this.document.getElementsByClassName('gm-style-iw-a') as HTMLCollectionOf<HTMLElement>;
    if (classObjList.length > 0) {
      for (let index = 0; index < classObjList.length; index++) {
        const element = classObjList[index];
        element.remove();
      }
    }
    // this.ngZoneService.run(() => {
    ...
    setTimeout(() => {
      ...
      _this.markerInfoWindow = infowindow;
      if (this.isGoogleMapMarkerListClicked) {
        google.maps.event.addListener(infowindow, 'domready', function () {
          _this.hotSameLatLongpropertyCount = _this.sameLatLongProperties.filter(t => t.RankingStatus == 'hot').length;
          // console.log("_this.hotSameLatLongpropertyCount", _this.hotSameLatLongpropertyCount);
          // if (_this.hotSameLatLongpropertyCount > 1) {
            // console.log("_this.sameLatLongProperties", _this.sameLatLongProperties);
            let properties = _this.sameLatLongProperties.filter(t => t.RankingStatus == 'hot');
            let orderedProperties = [];
            _this.maxRankedArr.forEach(element => {
              let property = _this.sameLatLongProperties.find(t => t.PropertyDetailId == element.PropertyDetailId);
              property ? orderedProperties.push(property) : false;
            });

            _this.minRankedArr.forEach(element => {
              let property = _this.sameLatLongProperties.find(t => t.PropertyDetailId == element.PropertyDetailId);
              property ? orderedProperties.push(property) : false;
            });

            let stableProperties = _this.sameLatLongProperties.filter(t => t.RankingStatus == 'stable');
            stableProperties.forEach(elementProp => {
              orderedProperties.push(elementProp);
            });
            _this.sameLatLongProperties = [];
            _this.sameLatLongProperties = JSON.parse(JSON.stringify(orderedProperties));
          // }
          // console.log("_this.sameLatLongProperties", _this.sameLatLongProperties);
          ...
          if (classObj.length > 0) {
            classObj[classObj.length - 1].classList.add('show');
          }

          let listTemplate = _this.document.getElementById('popListTemplate') as HTMLElement;
          if (listTemplate) {
            listTemplate.style.display = "block";
          }
          ...
          if (indexmarker < 0) {
            let array = _this.sameLatLongProperties.filter(t => t.PropertyDetailId != element.PropertyDetailId);
            for (let j = 0; j < array.length; j++) {
              let tempElem = array[j];
              lat = parseFloat(tempElem.LatitudeLongitude.split(",")[0]);
              lng = parseFloat(tempElem.LatitudeLongitude.split(",")[1]);
              indexmarker = _this.htmlMarkers.findIndex(t => t.lat == lat && t.lng == lng);
              if (indexmarker > 0) {
                break;
              }
            }
          ...
          if (indexmarker > -1) {
            ...
            if (rightList.length > 0 && arrowstyle.length > 0) {
              ...
              rightList[rightList.length - 1].style.display = "block";
              if (quadrant == "tr") {
                rightList[rightList.length - 1].style.left = "-146px";
                rightList[rightList.length - 1].style.top = "-40px";
                arrowstyle[rightList.length - 1].classList.add('arrow-list-tr');
              } else if (quadrant == "br") {
                rightList[rightList.length - 1].style.left = "-146px";
                rightList[rightList.length - 1].style.top = "-20px";
                arrowstyle[rightList.length - 1].classList.add('arrow-list-br');
              } else if (quadrant == "tl") {
                rightList[rightList.length - 1].style.top = "76px";
                rightList[rightList.length - 1].style.left = "146px";
                arrowstyle[rightList.length - 1].classList.add('arrow-list-tl');
              } else if (quadrant == "bl") {
                rightList[rightList.length - 1].style.left = "146px";
                rightList[rightList.length - 1].style.top = "20px";
                arrowstyle[rightList.length - 1].classList.add('arrow-list-bl');
              }
            }
          }
        });
      ...
      if (this.isDblClicked) {
        if (this.isGoogleMapMarkerClicked) {
  ...
  showMarkerPin(element?: Property) {
    ...
    if (element) {
      ...
      if (this.centerPolyLinePropertyId && element.PropertyId === this.centerPolyLinePropertyId) {
        ...
        let indexMarker = this.htmlMarkers.findIndex(t => t.lat == lat && t.lng == lng);
        if (indexMarker < 0) {
          let propList = this.properties.filter(t => t.LatitudeLongitude == element.LatitudeLongitude && t.PropertyDetailId != element.PropertyDetailId && element.IsSameLatLong == true);
          if (propList.length > 0) {
            let element1 = propList[0];
            let lat = parseFloat(element.LatitudeLongitude.split(",")[0]);
            let lng = parseFloat(element.LatitudeLongitude.split(",")[1]);
            if (element1) {
              indexMarker = this.htmlMarkers.findIndex(t => t.lat == lat && t.lng == lng);
            }
          }
        }
        if (indexMarker > -1) {
          ...
          if (markerElem) {
            ...
            if (propList.length > 0) {
              ...
              let lng = parseFloat(element.LatitudeLongitude.split(",")[1]);
              if (element1) {
                markerElem = this.document.getElementById("marker_" + this.pageName + "_" + element1.PropertyId);
                markerElem ? markerElem.style.zIndex = "3" : false;
                let divElem = this.document.getElementById("scaleDiv_" + this.pageName + "_" + element1.PropertyId)
                divElem ? divElem.style.transform = "scale(1.111)" : false;
              }
            }
          }
          ...
          if (propList.length > 0) {
            let element1 = propList[0];
            if (element1) {
              address = element1.Address + ", " + element1.Suburb + ", " + element1.City;
              index = this.AllMarkers.findIndex(t => t.title == address);
            }
          }
        }
      }
        ...
        if (indexMarker > -1) {
          ...
          if (markerElem) {
            ...
            if (propList.length > 0) {
              ...
              if (element1) {
                markerElem = this.document.getElementById("marker_" + this.pageName + "_" + element1.PropertyId);
                markerElem ? markerElem.style.zIndex = "1" : false;
                let divElem = this.document.getElementById("scaleDiv_" + this.pageName + "_" + element1.PropertyId)
                divElem ? divElem.style.transform = "scale(1)" : false;
              }
            }
          }
          ...
          if (maxRankedArr.length > 0) {
            ...
            maxRankedArr.forEach(element => {
              if (element.TotalRank < 4) {
                ...
                if (hotProperty) {
                  let isdrowLines = true;
                  if (coolingProperty.LatitudeLongitude == hotProperty.LatitudeLongitude) {
                    isdrowLines = false;
                  }
                  if (isdrowLines) {
              ...
              if (this.pageName === "SearchResultPage") {
                this.userPath ? this.polylines.push(this.userPath) : false;
                this.userPath ? this.userPath.setMap(this.map) : false;
              }
            });
          }
          this.setMarkerOnGoogleMap(this.map);
          ...
          if (minRankedArr.length > 0) {
            ...
            minRankedArr.forEach(element => {
              if (element.TotalRank < 4) {
                ...
                if (hotProperty1) {
                  let isdrowLines = true;
                  if (hotProperty1.LatitudeLongitude == hotProperty.LatitudeLongitude) {
                    isdrowLines = false;
                  }
                  if (isdrowLines) {
                    let rankedProperties: any[] = [];
      ...
      if (this.maxRankedArr.length > 0 && this.properties.length > 0) {
        ...
        if (countmax > 5) {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            property ? sortedProperties.push(property) : false;
          }
        } else {
          if (countmax > 3) {
            hotPropertyCount = 3;
        ...
        for (let index = 0; index < this.properties.length; index++) {
          ...
          if (existIndex < 0) {
            element ? sortedProperties.push(element) : false;
          }
        }
