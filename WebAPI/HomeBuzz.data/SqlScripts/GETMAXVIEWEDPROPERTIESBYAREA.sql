CREATE PROCEDURE [dbo].[GETMAXVIEWEDPROPERTIESBYAREA]
...
BEGIN
	...
	CREATE TABLE #RANKED(PropertyDetailId INT, Ranking INT, LastViewed DateTime)
	CREATE TABLE #RANKED_TEST(PropertyDetailId INT, Ranking INT, LastViewed DateTime)
	...
	IF(@TOTALPROPERTIES = 2 AND @TOTALVIEWS = 2)
		...
		INSERT INTO #VIEW30 
		SELECT TOP 1 PropertyDetailId,1 FROM #Last90Views order by ViewedDate DESC

		-- #VIEW30: insert rest properties with 0
		INSERT INTO #VIEW30 
		SELECT TOP 1 PropertyDetailId,0 FROM #Last90Views order by ViewedDate ASC

		-- #VIEW60: insert rest properties with 0
		INSERT INTO #VIEW60 
		SELECT TOP 1 PropertyDetailId,1 FROM #Last90Views order by ViewedDate ASC

		-- #VIEW60: insert first property with 0
		INSERT INTO #VIEW60 
		SELECT TOP 1 PropertyDetailId,0 FROM #Last90Views order by ViewedDate DESC
	...
	ELSE IF (@TOTALPROPERTIES >= 2 AND @TOTALVIEWS = 3)
		...
		-- #VIEW30: insert rest properties with 0
		INSERT INTO #VIEW30 
		SELECT TOP 2 PropertyDetailId,0 FROM #Last90Views order by ViewedDate ASC

		-- #VIEW60: insert rest properties with 0
		INSERT INTO #VIEW60 
		SELECT TOP 2 PropertyDetailId,1 FROM #Last90Views order by ViewedDate ASC
	...
	ELSE IF (@TOTALPROPERTIES >= 2 AND @TOTALVIEWS = 4)
		...
		-- #VIEW30: insert rest properties with 0
		INSERT INTO #VIEW30 
		SELECT TOP 3 PropertyDetailId,0 FROM #Last90Views order by ViewedDate ASC

		-- #VIEW60: insert rest properties with 0
		INSERT INTO #VIEW60 
		SELECT TOP 3 PropertyDetailId,1 FROM #Last90Views order by ViewedDate ASC
	...
	ELSE 
		...
		-- #VIEW30: insert first moste time stamped
		INSERT INTO #VIEW30 
		SELECT TOP (@33PER) PropertyDetailId,1 FROM #Last90Views order by ViewedDate DESC

		-- #VIEW60: insert rest properties with 0
		INSERT INTO #VIEW60 
		SELECT TOP (@66PER) PropertyDetailId,1 FROM #Last90Views order by ViewedDate ASC
		...
		INNER JOIN PropertyDetail pd ON R30.PropertyDetailId = pd.Id ORDER BY pd.ViewedDate DESC

		--select * from #RANKED
		--INSERT INTO #RANKED_TEST
		--select PropertyDetailId,Ranking,LastViewed from #RANKED order by LastViewed 

		--select * from #RANKED_TEST

		INSERT INTO #MAXRANKED
		Select TOP 1 PropertyDetailId, Ranking FROM #RANKED WHERE Ranking = (Select MAX(Ranking) from #RANKED) ORDER BY LastViewed DESC

		--SELECT  MIN(Ranking) FROM #RANKED WHERE Ranking IN(SELECT TOP 2 Ranking FROM #RANKED ORDER BY Ranking DESC) 
		--select * from #RANKED ORDER  BY Ranking DESC OFFSET  1 ROWS FETCH NEXT 1 ROWS ONLY 

		INSERT INTO #MINRANKED
		Select TOP 1 PropertyDetailId, Ranking FROM #RANKED Where Ranking = (Select MIN(Ranking) from #RANKED) ORDER BY LastViewed ASC

		INSERT INTO #FINALRANKED
		SELECT TOP 1 pd.PropertyId,mr.PropertyDetailId,1,pc.Latitude,pc.Longitude FROM #MAXRANKED mr 
		INNER JOIN PropertyDetail pd ON mr.PropertyDetailId = pd.Id
		INNER JOIN PropertyCrudData pc ON pd.Id = pc.PropertyDetailId
		order by ViewedDate desc

		INSERT INTO #FINALRANKED
		SELECT TOP 1 pd.PropertyId,mr.PropertyDetailId,0,pc.Latitude,pc.Longitude FROM #MINRANKED mr 
		INNER JOIN PropertyDetail pd ON mr.PropertyDetailId = pd.Id
		INNER JOIN PropertyCrudData pc ON pd.Id = pc.PropertyDetailId
		order by ViewedDate desc

		SELECT * from #FINALRANKED
END