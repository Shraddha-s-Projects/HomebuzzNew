...
namespace HomeBuzz.api {
    public class Startup {
        ...
        private readonly TimeWeightedPropertyFlowInterestLogic timeWeightedPropertyFlowInterest;
        ...
        public Startup (IHostingEnvironment env) {
            ...
            property = new DeactiveProperty (_propertyDetailService, _propertyOfferService, _propertyClaimService,
                ...
                _timeWeightedPropertyFlowInterestHourlyService,
                _timeWeightedPropertyFlowInterestDailyService,
                _timeWeightedPropertyFlowInterestWeeklyService, _userService,
                _emailTemplateService, context);

            propertyFlowInterest = new PropertyFlowInterestTimeIntervalLogic (
                _propertyFlowInterestHourlyService,
                _propertyFlowInterestDailyService,
                _propertyFlowInterestWeeklyService,
                _propertyViewService, _propertyDetailService, _propertyCrudDataService);

            timeWeightedPropertyFlowInterest = new TimeWeightedPropertyFlowInterestLogic (
                _timeWeightedPropertyFlowInterestHourlyService,
                _timeWeightedPropertyFlowInterestDailyService,
                _timeWeightedPropertyFlowInterestWeeklyService,
                _propertyViewService, _propertyDetailService, _propertyCrudDataService);
        }
        ...
        public void Configure (IApplicationBuilder app, IServiceProvider serviceProvider, IHostingEnvironment env, ILoggerFactory loggerFactory, HomeBuzzContext context, IBackgroundJobClient backgroundJobs) {
            using (var serviceScope = app.ApplicationServices.GetRequiredService<IServiceScopeFactory> ().CreateScope ()) {
                //HomeBuzzContext.ConnectionString = "Data Source=tcp:192.168.1.15\\TA15;Initial Catalog=Homebuzz;User Id=sa;Password=Admin@123;";
            ...
            RecurringJob.AddOrUpdate (
                () => timeWeightedPropertyFlowInterest.AddUpdate (FlowInterestType.Hourly.ToString ()),
                Cron.HourInterval (1));

            // Run every day at 00:00 AM
            RecurringJob.AddOrUpdate (
                () => timeWeightedPropertyFlowInterest.AddUpdateDaily (FlowInterestType.Daily.ToString ()),
                Cron.Daily (23, 59));

            RecurringJob.AddOrUpdate (
                () => timeWeightedPropertyFlowInterest.AddUpdateWeekly (FlowInterestType.Weekly.ToString ()),
                Cron.Weekly (DayOfWeek.Sunday, 23, 59));
