            ...
            try {
                ...
                if (searchVM != null) {
                    ...
                    if (propertyList.Count > 0) {
                        ...
                        foreach (var item in propertyList) {
                            var listOfSameLatLong = propertyList.Where (t => t.LatitudeLongitude == item.LatitudeLongitude).ToList ();
                            if (listOfSameLatLong.Count > 1) {
                                item.IsSameLatLong = true;
                            } else {
                                item.IsSameLatLong = false;
                            }
